...
echo "脚本所在路径${WORKDIR}"
PROFILE_PATH=${WORKDIR}/zookeeper/profile
...
zookeeper_file_sync(){
  echo "========开始zookeeper配置同步======"
  for((i=0;i<${#BASE_IP[@]};i++))
  do
      local_ip ${BASE_IP[i]}
      if [[ ${is_local_ip} -eq 0 ]] ;then
        echo "========本机ip跳过zookeeper配置同步======"
      else
        sh ${WORKDIR}/sync.sh ${BASE_USERNAME[i]}  ${BASE_IP[i]}
        rsync -avzP ${WORKDIR}/zookeeper ${BASE_USERNAME[i]}@${BASE_IP[i]}:${WORKDIR}/
      fi
  done
}
...
zookeeper_deploy(){
  ...
  do
    local_ip ${BASE_IP[i]}
    if [[ ${is_local_ip} -eq 0 ]] ;then
      bash ${WORKDIR}/zookeeper/zookeeper_install.sh ${i}
    else
      ssh ${BASE_USERNAME[i]}@${BASE_IP[i]} -t -t "bash ${WORKDIR}/zookeeper/zookeeper_install.sh ${i}"
    fi
  done
}
