...
echo "python脚本所在路径${WORKDIR}"
PROFILE_PATH=${WORKDIR}/python/profile
######################profile######################
...
python_file_sync(){
  ...
  do
      ...
      if [[ ${is_local_ip} -eq 0 ]] ;then
        echo "========本机ip跳过python安装文件同步======"
      else
        sh ${WORKDIR}/sync.sh ${BASE_USERNAME[i]}  ${BASE_IP[i]}
        rsync -avzP ${WORKDIR}/python ${BASE_USERNAME[i]}@${BASE_IP[i]}:${WORKDIR}/
      fi
  done
}
...
python_deploy(){
  ...
  do
    local_ip ${BASE_IP[i]}
    if [[ ${is_local_ip} -eq 0 ]] ;then
      bash ${WORKDIR}/python/python_install.sh
    else
      ssh ${BASE_USERNAME[i]}@${BASE_IP[i]} -t -t "bash ${WORKDIR}/python/python_install.sh"
    fi
  done
}
