...
echo "脚本所在路径${WORKDIR}"
PROFILE_PATH=${WORKDIR}/nginx/profile
...
nginx_file_sync(){
  ...
  do
      ...
      if [[ ${is_local_ip} -eq 0 ]] ;then
        echo "========本机ip跳过nginx配置同步======"
      else
        sh ${WORKDIR}/sync.sh ${BASE_USERNAME[i]}  ${BASE_IP[i]}
        rsync -avzP ${WORKDIR}/nginx ${BASE_USERNAME[i]}@${BASE_IP[i]}:${WORKDIR}/
      fi
  done
}
...
nginx_deploy(){
  ...
  do
    local_ip ${BASE_IP[i]}
    if [[ ${is_local_ip} -eq 0 ]] ;then
      bash ${WORKDIR}/nginx/nginx_install.sh
    else
      ssh ${BASE_USERNAME[i]}@${BASE_IP[i]} -t -t "bash ${WORKDIR}/nginx/nginx_install.sh"
    fi
  done
}
